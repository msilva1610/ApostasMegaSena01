...
def geraListas():
    global QtdePalpites
    global ListaDeQuinas
    global ListaPalpites
    global QtdePalpites
    global QtdeDeQuinas
    ListaDeQuinas = ListaDeQuinasSorteadas()
    ListaPalpites = ListaPalpites01()
    ...
    QtdePalpites = len(ListaPalpites)
    QtdeDeQuinas = len(ListaDeQuinas)
    print("Quantidade de palpites: {} e qtde de quinas: {}".format(QtdePalpites, QtdeDeQuinas))

def montaJson():
    with io.open('ListaDeQuinas.json', 'w', encoding='utf8', errors='ignore') as outfile:  
        json.dump(ListaDeQuinas, outfile, ensure_ascii=False)
    with io.open('ListaPalpites.json', 'w', encoding='utf8', errors='ignore') as outfile:  
        json.dump(ListaPalpites, outfile, ensure_ascii=False)

def dezenasQuinas(ListaDeQuinas,dezenas):
    qtde = 0
    for quina in ListaDeQuinas:
        # print("quinas: {}".format(quina))
        qtde = contaQuinas(dezenas,quina)
...
def teste(id):
    qtdeTernosSorteadas, qtdeQuadrasSorteadas, qtdeQuinasSorteadas, qtdeSenasSorteadas = 0,0,0,0
    palpite = ListaPalpites[id]
    id_aposta, dezenas = TrataCampo(palpite)
    with ThreadPoolExecutor(max_workers=20) as e:
        qtde = e.submit(dezenasQuinas(ListaDeQuinas,dezenas))

    # qtde = dezenasQuinas(ListaDeQuinas,dezenas)
    if qtde == 3:
        qtdeTernosSorteadas += 1
    elif qtde == 4:
        qtdeQuadrasSorteadas += 1
    ...
    result = {
        'id aposta': id_aposta,
        'dezenas': dezenas,
        'ternos': qtdeTernosSorteadas,
        'quadras': qtdeQuadrasSorteadas,
        'quinas': qtdeQuinasSorteadas
    }
    return result

def testeThread():
    total = len(ListaPalpites)
    with ThreadPoolExecutor(max_workers=800) as executor:
        for result in executor.map(teste, list(range(total))):
...
def calculaQuinas():
    for palpite in ListaPalpites:
        ...
        # print("id aposta: {} dezenas: {} ternos: {} quadras: {} quinas: {}".format(id_aposta,dezenas,qtdeTernosSorteadas,qtdeQuadrasSorteadas, qtdeQuinasSorteadas))
